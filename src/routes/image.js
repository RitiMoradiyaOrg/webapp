...
router.post('/:productId/image', authenticate, upload.single('image'), async (req, res) => {
  try {
...
router.get('/:productId/image/:imageId', authenticate, async (req, res) => {
  try {
...
router.delete('/:productId/image/:imageId', authenticate, async (req, res) => {
  try {
    const { productId, imageId } = req.params;
    const userId = req.user.user_id;
